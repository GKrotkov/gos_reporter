...
red_alliance <- c(3015, 3504, 5024)
blue_alliance <- c(836, 3937, 9609)
...
important_cols <- c(
    ...
    "harmony_success", "notes"
)

p1 <- matchdata[matchdata$team_id %in% 3015, important_cols]
p2 <- matchdata[matchdata$team_id %in% 3504, important_cols]
p3 <- matchdata[matchdata$team_id %in% 5024, important_cols]
o1 <- matchdata[matchdata$team_id %in% 836, important_cols]
o2 <- matchdata[matchdata$team_id %in% 3937, important_cols]
o3 <- matchdata[matchdata$team_id %in% 9609, important_cols]
...
```{r}
red_idx <- which(unlist(statbotics_data$team) %in% red_alliance)
blue_idx <- which(unlist(statbotics_data$team) %in% blue_alliance)

viz <- data.frame(
    team = unlist(statbotics_data$team)[c(red_idx, blue_idx)],
    tele = unlist(statbotics_data$teleop_epa_end)[c(red_idx, blue_idx)],
    auto = unlist(statbotics_data$auto_epa_end)[c(red_idx, blue_idx)], 
    endgame = unlist(statbotics_data$endgame_epa_end)[c(red_idx, blue_idx)], 
    color = c(rep("red", 3), rep("blue", 3))
)

ggplot(viz, aes(x = tele, y = auto + endgame, label = team, color = color)) + 
    geom_label() + 
    labs(title = "Statbotics Summary", 
         x = "Teleop EPA", y = "Auto + Endgame EPA")
```
