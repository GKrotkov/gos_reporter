...
event_code <- "2024ohcl" #change this to 2024ohcl Thursday
...
scouted <- read.delim("data/buckeye_qual_matches.txt")
...
```{r}
red_alliance <- c(1787, 3504, 8243)
blue_alliance <- c(3954, 8140, 4504)
...
"scouted_score" = blue_df$scouted_score, "Drive?" = blue_pitdf$DriveType, "Amp %" = blue_df$tele_amp_accuracy, "Can climb" = (blue_df$endgame_onstage_pct>0), "Under stage?" = (blue_pitdf$Height < 29), "Speaker %" = blue_df$tele_spk_accuracy, "IntakesFromGround?" = blue_pitdf$Ground_Pickup, "ShootsAwayFromSubwoofer" = blue_pitdf$Can_shoot_away_from_subwoofer, "Comments" = comments$team_number %in% blue_alliance)
...
"scouted_score" = red_df$scouted_score, "Drive?" = red_pitdf$DriveType, "Amp %" = red_df$tele_amp_accuracy, "Can climb" = (red_df$endgame_onstage_pct>0), "Under stage?" = (red_pitdf$Height < 29), "Speaker %" = red_df$tele_spk_accuracy, "IntakesFromGround?" = red_pitdf$Ground_Pickup, "ShootsAwayFromSubwoofer" = red_pitdf$Can_shoot_away_from_subwoofer, "Comments" = comments$team_number %in% red_alliance)
...
for (i in 1:length(vizTeamsList)){ 
    #print(paste("Team Number: ", vizTeamsList[i]))
    teamAutosData <- autoData[autoData$TeamNum==325] 
    #teamAutosData <- teamAutosData[order(teamAutosData$Time, decreasing = TRUE), ]
    #data(teamAutosData)
    head(teamAutosData)
    
    #giving up heres the same thing in excel
    #its easier than adding a DF to a PDF 
    write_xlsx(teamAutosData, "C:/Users/Anuva/Documents/gos_data_science_nogithub/gos_reporter_nogithub/data//trial.xlsx")
